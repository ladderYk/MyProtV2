...
#include "MyProt/Engine/AutoComputeProvider.hpp"  // ResolveDerivedLength (v1.11)
#include <chrono>
...
/// v1.22 → v1.25: 模板 raw 载荷标记与宽度表已上移 Engine (TemplateLayout.kRawMarker /
/// RequestBuilder::BuildTemplateLayout); InjectDerivedLengthVariables 同步上移,
/// Gateway 与 Service 试算校验共用同一实现 (ADR-0012 §1.2).

/// v1.9 增: 每次 Build/BuildBytes 前同步 autoCompute 规则到 _autoProvider.
///   - 空串 = 无规则, RequestBuilder 走 v1.8 退回路径 (Next 原子自增).
...
/// v1.9 增: 读回 Build 用 — 协议 autoComputeJson 已在 BuildWriteBackCheck
...
// v1.10 增: 从 op.inputs 中提取 source=static 且有值的条目, 转成 ctx.variables 形式.
//   auto 与无值的 static(原 hint, v1.15 并入) 不进入 ctx.variables (auto 走 AutoComputeProvider).
...
// v1.25: InjectDerivedLengthVariables 已上移 Engine::RequestBuilder (见文件头注释).
//   此处仅保留 op 级 autoIncrement 预解析 (ResolveAutoIncrementParameters).

// v1.16 增: 参数层预解析 — 把声明的 strategy=autoIncrement 变量在渲染前解析进参数表,
...
/// v1.10 增: 协议级 + op 级 autoCompute 合并 (公开 API, 对应 TagReader.hpp 声明).
/// v1.27: 实现上移至 Engine::RequestBuilder::MergeOpAutoComputeJson — 与 Service
///   试算校验共用同一实现 (原 Gateway/Service 双份且 derivedLength 处理已分叉)。
...
void TagReader::ReadBatch(const MergedRequest& merged,
    ...
    // 2. 构建合并请求的变量表
    //    v1.1: 协议 static 变量为基, 首个 tag 的 variables 覆盖同名键
    //    v1.10: 中间插入 op.inputs 静态覆盖 (协议 < op < 标签)
    //    v1.25: 不再注入 RegisterCount (协议族单位); 注入 ByteCount (跨协议字节单位).
    ...
    variables[Core::ByteCountVariableName()]      = merged.byteCount;  // v1.25 跨协议字节单位
    // v1.28: 位偏移为标签一等字段 — 注入 expr 作用域 (供协议 outputs derivedLength 引用)
    if (!tags.empty() && tags[0].bitOffset >= 0) {
        variables[Core::kBitOffsetExprVariable] = static_cast<uint32_t>(tags[0].bitOffset);
    }

    // v1.25: ReadBatch 路径补上 InjectDerivedLengthVariables 调用 (与 WriteBytes 路径一致).
    ...
    //    v1.9: 同步协议 autoCompute 规则 (空 = 不动)
    //    v1.10: 同时合并 op.inputs 中 source=auto 的覆盖项
    SyncAutoComputeRules(_autoProvider, protocol, op);
    // v1.16: 参数层预解析 — autoIncrement 渲染前进参数表 (帧感知 frameSlice/expr/crc 除外)
    ...
    channel.SendReceive(*requestShared, framing,
        ...
        [this, merged, tags, op, tagOrders, handler, requestShared](
            ...
            for (size_t i = 0; i < merged.tagIndices.size() && i < tags.size(); ++i) {
                ...
                //   v1.25 改: 字节地址差 = tag.StartAddress (协议 JSON derivedLength 派生前已是字节)
...
void TagReader::WriteOnce(const Core::TagDefinition& tag,
    ...
    //    v1.1: 协议 static 变量为基, 标签级 variables 覆盖同名键
    //    v1.6: 注入变量名由调用方指定 (写标签用 tag.writeVariable)
    //    v1.10: 插入 op 级 static 覆盖 (协议 < op < 标签)
    //    v1.25: 注入跨协议字节单位 StartByteAddress; 协议族地址 (如 Modbus StartAddress
    ...
    // v1.28: 位偏移为标签一等字段 — 注入 expr 作用域 (供位寻址派生, 如 Modbus FC05/FC15)
    if (tag.bitOffset >= 0) {
        variables[Core::kBitOffsetExprVariable] = static_cast<uint32_t>(tag.bitOffset);
    }

    // v1.25: 写路径派生长度注入 (与 ReadBatch/WriteBytes 一致).
    ...
    channel.SendReceive(*requestShared, framing,
        ...
        [this, op, framing, &channel, requestTimeoutMs,
         backCheck, writeRelease, requestShared, &writeSlot, protocol](
            ...
            // P1 B read-back (v1.6 泛化): 读 op/期望字节由调用方组装,
...
void TagReader::WriteBytes(const Core::TagDefinition& tag,
    ...
    //    v1.1: 协议 static 变量为基, 标签级 variables 覆盖同名键
    //    v1.10: 插入 op 级 static 覆盖 (协议 < op < 标签)
    //    v1.25: 注入跨协议字节单位 StartByteAddress (协议族地址由 outputs 派生, 见下方 Inject).
    std::unordered_map<std::string, uint32_t> variables =
        Engine::RequestBuilder::MergeVariables(
            Engine::RequestBuilder::CollectStaticVariables(protocol),
            OpStaticVariables(op),
    ...
    channel.SendReceive(*requestShared, framing,
        ...
        [this, op, writeRelease, requestShared, framing, &channel,
         requestTimeoutMs, backCheck, protocol](
            ...
            // P1 B read-back (v1.6 泛化): 与 WriteOnce 一致 — 读 op/期望
            // 字节由调用方组装。修复 v1.5 缺陷: 此处原复用写 op 模板
...
void TagReader::RunReadBack(Transport::IChannel& channel,
    ...
    // v1.9: 读回 Build 同步 autoCompute 规则 (BuildWriteBackCheck 已从源 protocol 快照)
